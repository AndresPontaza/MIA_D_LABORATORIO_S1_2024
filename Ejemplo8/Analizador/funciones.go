...
func mkfs(commandArray []string) {
	fmt.Println("[MENSAJE] El comando MKFS aqui inicia")
			...
			if aux != nil {
				...
				if index != -1 {
					...
					if err == nil {
						...
						fmt.Println("[MENSAJE] Formateando ", val_type)

						formatear_ext2(inicio, tamano, aux.Direccion)

						f.Close()
					} else {
						fmt.Println("[ERROR] No se puede abrir el archivo...")
					}

				} else {
					index = buscar_particion_l(aux.Direccion, aux.Nombre)
					fmt.Println("[MENSAJE] Index de la logica", index)
				}
			} else {
	...
	fmt.Println("[MENSAJE] El comando MKFS aqui finaliza")
}
...
func formatear_ext2(inicio int, tamano int, direccion string) {
	...
	if err == nil {
		...
		fmt.Println("[SUCCES] El Disco se formateo en el sistema EXT2 con exito!")
		f.Close()
	}
}
